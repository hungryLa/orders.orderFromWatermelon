...
XAAAoRAuAIBQ/gSIww4ZVui7KAAAAABJRU5ErkJggg=="/>
    ...
    <div>1.1. Продавец передает в собственность, а Покупатель принимает и оплачивает:</div>
        <div class="all_width_with_underline">{{$furnitures}}</div>
    ...
    <div>
        ...
        Договора в срок <div class="for_input_with_underline">{{\Carbon\Carbon::parse($order->deadline_delivery)->format('d.m.Y')?: '—'}}</div>. Качество Мебели должно соответствовать ГОСТ 16371-93 «Мебель - общие технические
    ...
    <div>
        3.1. Цена Договора включает в себя стоимость всей Мебели, передаваемой по настоящему Договору, и составляет <div class="for_input_with_underline">{{$order->price}}</div> рублей,
        ...
        3.3.1. Покупатель вносит авансовый платеж в размере <div class="for_input_with_underline">{{$order->deposit ?: 0}}</div> рублей, в том числе НДС не облагается
        с момента подписания Сторонами настоящего Договора.<br>
        3.3.2. Оставшаяся часть цены Договора в размере <div class="for_input_with_underline">{{$order->rest_amount ?: 0}}</div> рублей, в том числе НДС не облагается,
